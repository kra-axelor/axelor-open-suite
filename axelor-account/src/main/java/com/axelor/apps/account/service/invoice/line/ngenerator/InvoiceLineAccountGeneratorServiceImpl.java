...
public class InvoiceLineAccountGeneratorServiceImpl implements InvoiceLineAccountGeneratorService {
  ...
  public static class InvoiceLineAccountBuilder {
    ...
    private int discountTypeSelect;
    ...
    protected void completeTotals(Company company) throws AxelorException {
      if (typeSelect != InvoiceLineRepository.TYPE_TITLE) {
        if (!invoice.getInAti() && (exTaxTotal == null || inTaxTotal == null)) {
          ...
          inTaxTotal =
              exTaxTotal.add(exTaxTotal.multiply(taxRate)).setScale(2, RoundingMode.HALF_UP);
        } else {
   ...
   * {@inheritDoc}
   * </br>
   * This implementation uses InvoiceLineAccountBuilder to create the invoice line.
   * It is actually not necessary to call this method, and it is possible to directly use
   * the InvoiceLineAccountBuilder. This method is a remaining (it has the same parameters) 
   * of the constructor of this service predecessor {@link InvoiceLineGenerator} and it only exists 
   * to be able to create a invoice line in a classic way (calls a method of a injected service)
   */
  @Override
